...
宝塔传统方式下，配置文件路径为：`解压路径/application.properties`。

例如解压路径是 `/www/wwwroot/demo.zfile.vip`，则配置文件路径为 `/www/wwwroot/demo.zfile.vip/application.properties`。
...
4. 启动项目，访问验证。（一般启动需要 1-3 分钟，访问如果还是之前的版本，请清除浏览器缓存！）
