...
wget https://c.jun6.net/ZFILE-PRO/zfile-pro-release_linux_amd64.tar.gz  # 下载 zfile 最新版
...
export ZFILE_INSTALL_PATH=~/zfile-pro                                   # 声明安装到的路径
mkdir -p $ZFILE_INSTALL_PATH && cd $ZFILE_INSTALL_PATH                  # 创建文件夹并进入
wget https://c.jun6.net/ZFILE-PRO/zfile-pro-release_linux_arm.tar.gz    # 下载 zfile 最新版
...
```bash showLineNumbers
~/zfile-pro/WEB-INF/classes/application.properties
```
